...
const ChatLabel = ({ openMenu, setOpenMenu, id, name }) => {
    ...
    const renameHandler = async () => {
        try {
            ...
            if (!newName) return
            const { data } = await axios.post('/api/chat/rename', { chatId: id, anem: newName })
            if (data.success) {
                fetchUsersChats()
    ...
    return (
        <div onClick={selectChat}
            ...
            <div onClick={e => { e.stopPropagation(); setOpenMenu({ id: id, open: !openMenu.open }) }}
                ...
                <Image src={assets.three_dots} alt='' className={`w-4 ${open.id === id && openMenu.open ? '' : 'hidden'} group-hover:block`} />
                <div className={`absolute ${open.id === id && openMenu.open ? 'block' : 'hidden'} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}>
